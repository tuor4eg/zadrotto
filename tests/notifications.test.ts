...
const transportOutboxSource = readFileSync("src/db/queries/notification-transport-outbox.ts", "utf8")
const transportDeliverySource = readFileSync("src/lib/notifications/outbox-delivery.ts", "utf8")
const transportPolicySource = readFileSync("src/lib/notifications/outbox-policy.ts", "utf8")
const transportOutboxMigrationSource = readFileSync("drizzle/0085_notification_transport_outbox.sql", "utf8")
...
describe("notification consumer", () => {
  ...
  it("fans out to external transports through a retryable transactional outbox", () => {
    assert.match(consumerSource, /insert\(notificationTransportOutbox\)[\s\S]*onConflictDoNothing/)
    assert.doesNotMatch(consumerSource, /fetch\(|TelegramTransport|api\.telegram/)
    assert.match(transportOutboxMigrationSource, /notification_transport_outbox_delivery_idx/)
    assert.match(transportOutboxMigrationSource, /'notification-transport-delivery'[\s\S]*'\* \* \* \* \*'/)
    assert.match(transportOutboxSource, /\.for\("update", \{ skipLocked: true \}\)/)
    assert.match(transportOutboxSource, /attempts: sql`\$\{notificationTransportOutbox\.attempts\} \+ 1`/)
    assert.match(transportOutboxSource, /leaseToken = randomUUID\(\)/)
    assert.match(transportOutboxSource, /eq\(notificationTransportOutbox\.leaseToken, leaseToken\)/)
    assert.match(transportOutboxSource, /eq\(notificationTransportOutbox\.status, "sending"\)/)
    assert.match(transportOutboxMigrationSource, /PRIMARY KEY\("event_id","transport","recipient"\)/)
    assert.match(transportPolicySource, /attempts >= NOTIFICATION_TRANSPORT_MAX_ATTEMPTS \? "failed" as const : "pending" as const/)
    assert.match(transportDeliverySource, /sendClaimedMessage\(message, signal\)/)
    assert.match(externalSource, /getEnabledExternalTransportCodes\(event\.type\)/)
    assert.match(externalSource, /new TelegramTransport/)
    assert.match(externalSource, /if \(!transport\.isReady\(\)\) throw new Error/)
    assert.match(externalSource, /throw new Error\(`Telegram notification failed/)
