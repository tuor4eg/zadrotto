...
Demo-ачивки — отдельная непостоянная проекция гостевой истории. Для них не создавать domain events,
outbox, серверные выдачи или строки прогресса: сервер отдаёт только разрешённый каталог и вычисляемое
состояние, а announced/toast state хранится локально и очищается вместе с demo-профилем. Любой
принимаемый сервером список code повторно ограничивать опубликованными и доступными записями.
Для изменений этой ветки дополнительно использовать `user-state-modes`.
