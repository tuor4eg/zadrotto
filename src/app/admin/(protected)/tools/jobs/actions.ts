...
export async function createJobAction(formData: FormData) {
  ...
  try {
    const job = await createManagedPeriodicJob({ code: String(formData.get("code") ?? ""), cronExpression: String(formData.get("cronExpression") ?? ""), payload: JSON.parse(String(formData.get("payload") ?? "{}")), policy: { maxAttempts: integer(formData.get("maxAttempts"), 3), retryBaseSeconds: integer(formData.get("retryBaseSeconds"), 60), retryMaxSeconds: integer(formData.get("retryMaxSeconds"), 3600), timeoutSeconds: integer(formData.get("timeoutSeconds"), 300) }, type: String(formData.get("type") ?? "") });
...
export async function updateJobAction(formData: FormData) {
  ...
  if (!Number.isInteger(jobId) || jobId <= 0) redirect(`${PATH}?error=invalid`);
  try {
    const job = await updateManagedPeriodicJob(jobId, { code: String(formData.get("code") ?? ""), cronExpression: String(formData.get("cronExpression") ?? ""), enabled: formData.get("enabled") === "true", payload: JSON.parse(String(formData.get("payload") ?? "{}")), policy: { maxAttempts: integer(formData.get("maxAttempts"), 3), retryBaseSeconds: integer(formData.get("retryBaseSeconds"), 60), retryMaxSeconds: integer(formData.get("retryMaxSeconds"), 3600), timeoutSeconds: integer(formData.get("timeoutSeconds"), 300) }, type: String(formData.get("type") ?? "") });
...
export async function retryJobRunAction(formData: FormData) { const admin = await requireAdminUser(); const runId = id(formData.get("id")); const run = await retryFailedJobRun(runId, admin.id); if (run) await logActivity({ action: "job-run.retry-requested", actorType: "admin", adminUserId: admin.id, entityType: "job-run", entityId: run.id, entityLabel: run.type, message: "Повторный запуск поставлен в очередь." }); revalidatePath(PATH); redirect(PATH); }
export async function cancelJobRunAction(formData: FormData) { const admin = await requireAdminUser(); const runId = id(formData.get("id")); const run = await cancelQueuedJobRun(runId); if (run) await logActivity({ action: "job-run.cancelled", actorType: "admin", adminUserId: admin.id, entityType: "job-run", entityId: run.id, entityLabel: run.type, message: "Запуск фоновой задачи отменён." }); revalidatePath(PATH); redirect(PATH); }
