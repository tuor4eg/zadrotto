...
import { ArchiveRiddle } from "./archive-riddle";
import { ArchiveFeed } from "./archive-feed";
import { AdaptiveReviewExcerpt } from "./adaptive-review-excerpt";
...
function EditorialCollectionsStrip({ collections }: { collections: PublishedCollection[] }) {
  ...
  return <section className="archive-paper archive-panel overflow-hidden p-3 sm:p-4" aria-labelledby="test-collections-title">
    <div className="mb-3 flex items-center justify-between gap-4">
      <h2 id="test-collections-title" className="flex items-center gap-2 font-serif text-2xl leading-none text-stone-950"><Layers3 className="size-5 text-red-950/70" aria-hidden="true" />Подборки</h2>
...
function DailyRecommendation({
  ...
  return (
    <section
      ...
      aria-labelledby="test-daily-recommendation"
    >
      {item ? (
        <Link
      ...
      <div className="relative z-10 flex flex-1 max-w-[72%] flex-col">
        <div className="flex h-8 shrink-0 items-start gap-2">
          ...
          <h2 id="test-daily-recommendation" className="font-serif text-2xl leading-none text-stone-950">
            Рекомендуем сегодня
          </h2>
        </div>
...
function LatestReview({
  ...
  return (
    <section
      className="archive-paper archive-panel group relative flex min-h-[280px] flex-col overflow-hidden p-3 sm:p-4 lg:h-[280px] lg:min-h-0"
      aria-labelledby="test-latest-review"
    >
      {review ? (
        <Link
      ...
      <div className="relative z-10 flex flex-1 max-w-[72%] flex-col">
        <div className="flex h-8 shrink-0 items-start gap-2">
          ...
          <h2 id="test-latest-review" className="font-serif text-2xl leading-none text-stone-950">
            Мнение из архива
          </h2>
        </div>
        <p className="mt-1 h-8 shrink-0 text-sm leading-6 text-stone-600">
        ...
        <div className={hasLatestActivity ? "grid gap-3 lg:grid-cols-[minmax(0,1fr)_17rem]" : undefined}>
          ...
          <div className="max-w-2xl">
            ...
            <p className="mt-4 max-w-xl text-base leading-7 text-stone-700 sm:text-lg">
              Оценивай, создавай личные серии и подборки и смотри, как из этого складывается твоя история.
            </p>
            <button
              type="button"
