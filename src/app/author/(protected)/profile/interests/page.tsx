...
import { AuthorToasts, type AuthorToast } from "@/app/author/(protected)/author-toasts";
import {
  resetAuthorMediaTypeSettingsAction,
  saveAuthorMediaTypeSettingsAction,
} from "@/app/author/(protected)/settings/media-types/actions";
import { Alert } from "@/components/ui/alert";
import { Button } from "@/components/ui/button";
import { getEffectiveMediaTypeOptions } from "@/db/queries/media-types";
import { requireAuthor } from "@/lib/auth/author-auth";

type AuthorInterestsPageProps = {
  searchParams: Promise<{ error?: string; reset?: string; saved?: string }>;
};
...
export default async function AuthorInterestsPage({ searchParams }: AuthorInterestsPageProps) {
  ...
  return (
    <div className="grid gap-6">
      <div>
        <h2 className="font-serif text-3xl leading-none text-stone-950">Интересующие типы медиа</h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">
          Выберите типы медиа, которые хотите видеть в каталоге, поиске, сериях, тегах и рекомендациях.
        </p>
      </div>

      <AuthorToasts clearParams={["error", "reset", "saved"]} messages={toast ? [toast] : []} />
      ...
      <form action={resetAuthorMediaTypeSettingsAction}>
        <Button type="submit" variant="outline"><RotateCcw />Вернуть значения по умолчанию</Button>
      </form>
    </div>
  );
}
