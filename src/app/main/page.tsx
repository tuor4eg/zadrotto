...
async function RandomDossierLink({ promise }: { promise: Promise<MainPageMediaItem | null> }) {
  ...
  return <Link href={item ? `/media/${item.code}` : "/"} className="archive-control-surface inline-flex h-10 items-center justify-center rounded-md border border-stone-300/80 px-4 font-mono text-xs uppercase tracking-wider hover:border-stone-700"><Shuffle className="mr-2 size-4" />Случайное досье</Link>;
}
...
async function AboutArchive({ promise }: { promise: ReturnType<typeof createMainPageDataPromises>["about"] }) {
  ...
  return <div className="divide-y divide-dashed divide-stone-400/35"><Link href="/" className="group flex items-center justify-between py-3"><span className="font-mono text-xs uppercase tracking-wider text-stone-600 group-hover:text-stone-950">Всего записей</span><strong className="font-serif text-2xl">{totalCount.toLocaleString("ru-RU")}</strong></Link>{counts.map((item) => { const MediaTypeIcon = MEDIA_TYPE_ICONS[item.mediaType] ?? Archive; return <Link key={item.mediaType} href={`/?type=${encodeURIComponent(item.mediaType)}`} className="group flex items-center justify-between gap-4 py-3"><span className="flex items-center gap-2 font-serif text-lg"><MediaTypeIcon aria-hidden="true" className="size-4 text-red-950/65 transition-colors group-hover:text-red-950" />{item.mediaTypeName}</span><span className="font-mono text-sm tabular-nums text-stone-600 group-hover:text-stone-950">{item.count.toLocaleString("ru-RU")}</span></Link>; })}</div>;
}
      ...
      <div className="mx-auto flex w-full max-w-[1480px] flex-col gap-3">
        ...
        <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_320px] xl:grid-rows-[auto_auto_auto]">
          <div className="flex min-w-0 flex-col gap-3 xl:contents">
            <Section className="xl:col-start-1 xl:row-start-1" href="/?sort=average_score" icon={<Star className="size-5" />} title="Топ архива"><Suspense fallback={<MainSectionLoader minHeight="min-h-52" />}><SectionItems promise={data.top} /></Suspense></Section>
            <div className="grid gap-3 lg:grid-cols-2 xl:col-start-1 xl:row-start-2">
              <Section href="/?sort=created_at" icon={<Clock3 className="size-5" />} title="Новое в базе"><Suspense fallback={<MainSectionLoader />}><SectionItems promise={data.newItems} /></Suspense></Section>
            ...
            {currentAuthor ? (
              <div className="grid gap-3 lg:grid-cols-2 xl:col-start-1 xl:row-start-3">
                <Section href="/?sort=my_rating_date&mine=rated" icon={<Star className="size-5" />} title="Мои последние оценки"><Suspense fallback={<MainSectionLoader />}><SectionItems promise={data.latestRatings} /></Suspense></Section>
                <Section href="/?mine=wanted" icon={<Bookmark className="size-5" />} title="Желаемое"><Suspense fallback={<MainSectionLoader />}><SectionItems promise={data.wanted} /></Suspense></Section>
              </div>
            ) : (
