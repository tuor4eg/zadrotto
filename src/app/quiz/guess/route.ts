...
export async function POST(request: Request) {
  if (!(await getCurrentAuthor())) return Response.json({ error: "Требуется авторизация." }, { status: 401 });
  let titleId: unknown;
  ...
  const result = await checkQuizGuess(Number(titleId));
  if (result.kind === "missing") return Response.json({ error: "Активный квиз не найден." }, { status: 404 });
  if (result.kind === "title-missing") return Response.json({ error: "Запись не найдена." }, { status: 404 });
  if (result.kind === "invalid-type") return Response.json({ error: "Этот тип записи не участвует в квизе." }, { status: 400 });
